...
from app.services.whatsapp import send_booking_confirmation, send_noshow_rebook
...
class AppointmentCreate(BaseModel):
    ...
    def validate_time(cls, v: str) -> str:
        try:
            appt_dt = datetime.fromisoformat(v)
            if appt_dt.tzinfo is None:
                appt_dt = appt_dt.replace(tzinfo=timezone.utc)
            if appt_dt <= datetime.now(timezone.utc):
                raise ValueError("Appointment time must be in the future")
        except ValueError as e:
            raise ValueError(f"Invalid ISO format or past time: {str(e)}")
        return v
...
def get_appointments(clinic_id: str, status: Optional[str] = None, date: Optional[str] = None, page: int = 1, per_page: int = 50, current_clinic=Depends(get_current_clinic)):
    ...
    supabase = get_supabase()
    query = supabase.table("appointments").select("*, patients(name, phone)").eq("clinic_id", clinic_id).order("appointment_time")
    
    if status:
        query = query.eq("status", status)
    if date:
        query = query.gte("appointment_time", f"{date}T00:00:00").lte("appointment_time", f"{date}T23:59:59")
...
def mark_no_show(appointment_id: str, current_clinic=Depends(get_current_clinic)):
    ...
    appt = supabase.table("appointments").select("clinic_id, patients(name, phone), clinics(name)").eq("id", appointment_id).execute()
    
    if not appt.data:
        raise HTTPException(status_code=404, detail="Appointment not found")
    
    if appt.data[0]["clinic_id"] != current_clinic["id"]:
        raise HTTPException(status_code=403, detail="Access denied")
...
def complete_appointment(appointment_id: str, current_clinic=Depends(get_current_clinic)):
    ...
    existing = supabase.table("appointments").select("clinic_id").eq("id", appointment_id).execute()
...
def trigger_noshow_manually(background_tasks: BackgroundTasks, current_clinic=Depends(get_current_clinic)):
    ...
    return {"success": True, "message": "No-show check triggered"}
